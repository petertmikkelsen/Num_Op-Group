...
def Newton(x0, f, df, ddf, tol, maxiter):
    x = x0
    x_list = []
    x_list.append(x)
    for i in range(maxiter):
        d = -np.linalg.inv(ddf(x)) @ df(x)
        alpha = BacktrackingLineSearch(x, d, f, df, 'Newton')
        x = x + alpha * d
        x_list.append(x)
        # check if the norm of the gradient is smaller than the tolerance
        if np.linalg.norm(df(x) - x_opt(f.__name__, 2)) < tol:
...
if __name__ == '__main__':
    ...
    x0 = np.random.uniform(-2, 2, (nb_run, dim))
    ...
    for n in range(len(functions)):
        ...
        for i in range(nb_run):
            ...
            x_list = Newton(x0[i], functions[n], dfuntions[n], ddfuntions[n], epsilon, maxiter)
